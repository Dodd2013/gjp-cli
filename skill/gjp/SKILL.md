---
name: gjp
description: 通过 gjp CLI 操作网上管家婆（wsgjp）进销存系统——开销售/采购单（含退货）、开费用单、查库存、管商品、管客户/供应商、单据中心查历史单据、查应收应付/对账、收付款单、费用单、查利润报表等。
  当用户提到「管家婆」「进销存」「开单/开销售单/开采购单」「销售退货/采购退货」「查库存/库存状况/库存明细」「新增商品/建商品」「客户/供应商/往来单位」「查单据/单据中心/历史单据」「应收/应付/对账」「付款单/收款单/费用单」「利润/报表」「出入库」「gjp」等，需要在该系统里做操作时使用。
...
### 创建费用单（XFY- 前缀，费用科目 + 付款）

gjp finance fee -p <往来单位名> -s <费用科目名> --amount <金额> [-t 管理] [-a 现金] [--memo 摘要] [--date YYYY-MM-DD] [--dry-run]

- 费用科目：运费/佣金/手续费/快递费/水电费…（科目档案，parTypeId=0000400003）
- `-t` 费用性质：采购|销售|管理|库存（默认 管理）
- `-a` 付款账户（默认 现金）；`-a none` = 不付款挂应付
- 多行费用：`-l "运费:100:上海发运;手续费:20"`
- `--no-post` 仅存草稿不过账；`--dry-run` 仅解析名称→ID
- **输出**：`{success, billNumber, vchcode, total, party, customType, lines, account, posted}`

例：登记快递费 100 元 → `gjp finance fee -p 顺丰速运 -s 快递费 --amount 100 --memo 本月运费`
